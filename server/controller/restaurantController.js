const resturantModel = requie("../model/restaurantModel.js");


const createResturant = async  (req, res) => {
    try{

        const {res_name, res_email, res_mobile,res_Owner_Name,res_Owner_email,res_Owner_mobile,res_Owner_streetAddress,res_Owner_stateProvince,res_Owner_postalCode,res_Owner_country,img}= req.body;
        if(!res_name  || !res_email || !res_mobile){
            return res.status(400).json({"message": "All fields are required Please provide all the details."});
        }else{
...
const updateResturant = async (req,res)=>{
    ...
    try{
         let updatedData =await resturantModel.findByIdAndUpdate(req.params.id , {res_name, 
            res_email, 
            res_mobile,
            res_Owner_Name,
            res_Owner_email,
...
module.exports={getAllResturants,createResturant,updateResturant,deleteResturent}
