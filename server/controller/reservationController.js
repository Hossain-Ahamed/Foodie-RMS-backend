...
const createReservation = async (req, res) => {
  ...
  const confirmReservation = async (req, res) => {
    try {
      ...
      const message = `Dear ${c.user_name},%0AWe're thrilled to inform you that your reservation at ${rest.res_name} has been successfully approved! Your table is now reserved for ${c.date} at ${c.timeSlot}, and we can't wait to host you for a delightful dining experience.`
      const smsResponse = await axios.post(
          `https://bulksmsbd.net/api/smsapi?api_key=${process.env.BULK_MESSAGE_API}&type=text&number=${cleanedNumber}&senderid=${process.env.BULK_MESSAGE_SENDER}&message=${message}`
        );
  ...
  const cancelAndDeleteReservation = async (req, res) => {
    try {
      ...
      const rest = await Restaurant.findOne({ _id: res_id });
      let cleanedNumber = c.user_phone.replace(/^\+88/, '');
      const message = `Dear ${c.user_name},%0AWe hope this message finds you well. We regret to inform you that your reservation at ${rest.res_name} for ${c.date} at ${c.timeSlot} has been canceled.`
      const smsResponse = await axios.post(
          `https://bulksmsbd.net/api/smsapi?api_key=${process.env.BULK_MESSAGE_API}&type=text&number=${cleanedNumber}&senderid=${process.env.BULK_MESSAGE_SENDER}&message=${message}`
        );
      await Reservation.findByIdAndDelete(_id);
