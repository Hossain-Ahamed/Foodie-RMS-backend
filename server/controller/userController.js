...
const signIn =async(req,res)=>{
    ...
    if(!user){responseError(res,200,"Invalid Email or Password")}
    else{
...
const JWTtoken = async(req,res)=>{
    ...
    try {
              ...
              const accesstoken = await JWT.sign({ _id: user._id,email:email,firebase_UID:firebase_UID }, process.env.JWT_SECRET_TOKEN);
...
const checktoken = (req, res, next) => {
  ...
  const getProfile = async (req,res)=> {
    let user=await userModel.findById(req.params.userID,"-password").lean().exec() ;
    if(!user){
